...
// Lê imagem PGM, retorna buffer alocado com pixels (0..tons) e preenche linhas/colunas/tons.
unsigned char *lePgm(const char *filename, int *out_linhas, int *out_colunas, int *out_tons) {
    ...
    if (strcmp(tipo, "P2") != 0) {
        printf("Formato não suportado. Use P2.\n");
        fclose(f);
        return NULL;
    }
...
// Escreve PGM a partir de buffer
...
void menu() {
    while (1) {
        ...
        printf("7 - Compactar banco de imagens\n");
        printf("8 - Reconstruir média\n");
        printf("0 - Sair\n");
        printf("Opcao: ");
        int opc;
